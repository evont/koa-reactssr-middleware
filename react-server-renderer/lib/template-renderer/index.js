...
var createMapper = require("./create-async-file-mapper");
var parseTemplate = require("./parse-template");
var templateStream = require("./template-stream");
...
TemplateRenderer.prototype.renderSync = function (content, context) {
    ...
    if (this.inject) {
        ...
        if (asyncContext) {
            content += "<script>window.ASYNC_COMPONENTS_STATE=" + serializeJS(asyncContext.getState()) + "</script>";
        }
...
TemplateRenderer.prototype.renderStyles = function (context) {
    ...
    var cssFiles = this.clientManifest
        ? this.clientManifest.all.filter(util.isCSS)
        : [];
    return (
...
TemplateRenderer.prototype.renderState = function (context, options) {
    ...
    var state = serializeJS(context[contextKey], { isJSON: true });
